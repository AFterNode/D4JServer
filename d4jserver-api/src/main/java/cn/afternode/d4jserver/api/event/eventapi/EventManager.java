package cn.afternode.d4jserver.api.event.eventapi;
...
import cn.afternode.d4jserver.api.event.eventapi.events.EventStoppable;
import cn.afternode.d4jserver.api.event.eventapi.types.Priority;
import cn.afternode.d4jserver.api.utils.SecurityUtils;
...
public final class EventManager {
	...
	public EventManager(String id) {
        this.id = id;
        this.logger = LogManager.getLogger("EventManager/" + id);
	}
     ...
     * a new MethodData to key's matching list and sorts it based on Priority. @see cn.afternode.d4jserver.api.event.eventapi.types.Priority
    ...
    private void invoke(MethodData data, Event argument) {
        try {
            ...
            logger.warn("Cannot pass event " + argument.getClass().getName() + " to " + data.getSource().getClass().getName() + "#" + data.getTarget().getName());
        } 
    }

    public void unregisterAll() {
        SecurityUtils.check(SecurityUtils.walker.getCallerClass());
    }
